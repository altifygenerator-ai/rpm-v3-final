CREATE OR REPLACE FUNCTION public.customer_update_project_status(p_token_hash text, p_status text, p_hired_contractor_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(lead_id uuid, outcome_status lead_outcome_status, marketplace_status lead_marketplace_status)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
begin
  ...
  if p_status = 'hired' then
    ...
    on conflict on constraint lead_outcomes_pkey do update
      set status='hired',
  ...
  update public.customer_project_access as access_row
  set last_used_at=now()
  where access_row.lead_id=v_lead_id;
