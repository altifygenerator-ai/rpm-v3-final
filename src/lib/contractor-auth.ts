...
export async function getContractorContext(): Promise<ContractorContext | null> {
  ...
  const { data: profile, error } = await admin
    ...
    .select(
      "id,user_id,email,business_name,slug,contact_name,phone,website_url,facebook_url,description,logo_url,city,state,zip,status,access_role,public_profile_enabled,featured,onboarding_completed"
    )
    .eq("user_id", user.id)
    .maybeSingle();
